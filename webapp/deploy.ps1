...
param(
    ...
    # Control-panel runtime config (written to webapp/config.js before publish).
    # BackendUrl defaults to the ca-simulator ingress FQDN (auto-discovered).
    [string]$BackendUrl,
    [string]$ContainerApp = "ca-simulator",
    [string]$TenantId,
    [string]$ClientId,
    [string]$Scope,
    [string]$MsalVersion = "2.38.4"
)
...
# ---------------------------------------------------------------------------
# 3b. Generate webapp/config.js + vendor MSAL (repeatable, no manual entry)
# ---------------------------------------------------------------------------
if (-not $TenantId) { $TenantId = $env:SIM_AUTH_TENANT_ID; if (-not $TenantId) { $TenantId = $env:FABRIC_TENANT_ID } }
if (-not $ClientId) { $ClientId = $env:SIM_AUTH_CLIENT_ID }
if (-not $ClientId) { throw "ClientId not provided and SIM_AUTH_CLIENT_ID missing in .env (run scripts/setup-app-registration.ps1 first)" }
if (-not $Scope)    { $Scope = "api://$ClientId/access_as_user" }

if (-not $BackendUrl) {
    Write-Host "[swa] discovering backend FQDN from container app '$ContainerApp'" -ForegroundColor Cyan
    $fqdn = az containerapp show -g $RgName -n $ContainerApp `
        --query "properties.configuration.ingress.fqdn" -o tsv 2>$null
    if (-not $fqdn) { throw "Could not discover ingress FQDN for '$ContainerApp'. Pass -BackendUrl explicitly." }
    $BackendUrl = "https://$fqdn"
}
Write-Host "[swa] backend: $BackendUrl" -ForegroundColor Green

$configJs = @"
"use strict";

// AUTO-GENERATED by webapp/deploy.ps1 — do not edit by hand.
// Access is gated by Entra ID sign-in: only tenant users assigned to the app
// registration can obtain a token and reach the control API.
const CONFIG = {
...
"@
Set-Content -LiteralPath (Join-Path $PSScriptRoot "config.js") -Value $configJs -Encoding utf8
Write-Host "[swa] wrote webapp/config.js" -ForegroundColor Green

# Vendor MSAL.js locally (the public CDN path is unreliable / 404s).
$vendorDir = Join-Path $PSScriptRoot "vendor"
$msalPath  = Join-Path $vendorDir "msal-browser.min.js"
if (-not (Test-Path $msalPath)) {
    New-Item -ItemType Directory -Force $vendorDir | Out-Null
    $msalUrl = "https://cdn.jsdelivr.net/npm/@azure/msal-browser@$MsalVersion/lib/msal-browser.min.js"
    Write-Host "[swa] vendoring MSAL $MsalVersion -> webapp/vendor/" -ForegroundColor Cyan
    Invoke-WebRequest $msalUrl -OutFile $msalPath -UseBasicParsing
}
...
Write-Host "[swa] Access is gated by Entra ID sign-in (assigned users only)." -ForegroundColor Yellow
Write-Host "[swa] Ensure the SPA redirect URI https://$hostname is registered on the app" -ForegroundColor Yellow
Write-Host "      (scripts/setup-app-registration.ps1 -SpaRedirectUris @('https://$hostname','http://localhost:4280'))." -ForegroundColor Yellow
