...
- The cloud simulator (Container App `ca-simulator` in `rg-fabric-demo`)
  runs `SIM_MACHINES=4` with the real CNC machine pinned explicitly via
  ...
  `simulator-cloud/src/simulate_machines.py`). M-002 is driven by the
  synthgen replay trace via `SIM_SYNTH_MACHINE=M-002` +
  `SIM_SYNTH_PROFILE=/app/synth_trace_M-002.json` (a `SynthMachine` path that
  loops a 1 Hz synthetic CNC trace generated by `tools/build_synth_trace.py`
  from the real spindle telemetry).
